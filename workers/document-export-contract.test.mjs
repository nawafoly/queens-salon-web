...
test("active attendance and staff performance reports use Export V2 instead of legacy browser-print PDF", () => {
  ...
  assert.match(attendance, /DOCUMENT_BRANDING/);
  ...
  assert.match(performance, /extraTables/);
  assert.match(performance, /bookingDetails/);
  assert.match(performance, /topServices/);
  assert.doesNotMatch(performance, /common\.ts|window\.print|exportHtmlDocumentToPdf/);
});
test("legacy report common runtime and dead payroll exporters stay retired", () => {
  ...
  const payrollV2 = read("src/helpers/reports/exportPayrollReportV2.ts");
  ...
  assert.doesNotMatch(payrollLegacySurface, /exportPayrollReportPdf|exportPayrollReportExcel|exportHtmlDocumentToPdf|window\.print|common\.ts/);
  assert.match(payrollV2, /exportReportToPdfV2/);
  assert.match(payrollV2, /exportReportToExcelV2/);
});
