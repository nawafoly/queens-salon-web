...
function mapDiscount(row: Record<string, unknown>): CoreDiscount {
  return {
    ...
    priceBeforeHalalas: row.price_before_halalas == null ? null : Number(row.price_before_halalas),
    priceAfterHalalas: row.price_after_halalas == null ? null : Number(row.price_after_halalas),
    published: Number(row.published ?? 1) === 1 || row.published === true,
    status: String(row.status || (Number(row.active) === 1 || row.active === true ? "active" : "disabled")),
    ...
    targetClientIds: parseArray(row.target_client_ids_json).map(String),
