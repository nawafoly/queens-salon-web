...
const FixedFaCommentDots = (props: IconBaseProps) => <FaCommentDots {...props} />;
...
function getAllBookings(): BookingItem[] {
    return safeJsonParse<BookingItem[]>(localStorage.getItem(STORAGE.BOOKINGS), []);
}
...
const isGreeting = (qRaw: string) => {
    ...
    return /(السلام عليكم|وعليكم السلام|هلا|هلا والله|اهلين|اهلا|مرحبا|مرحب|هاي|hi|hello)/i.test(q);
};
...
const isComplaint = (qRaw: string) => {
    const q = normalizeArabic(qRaw.toLowerCase());
    return /(سيء|سيئ|مو زين|زفت|خايس|يزعل|مضايق|ما عجبني|خدمتكم سيئه|تجربه سيئه)/i.test(q);
};
...
const ChatBot: React.FC = () => {
    ...
    const pageActions: Action[] = useMemo(() => {
        ...
        if (path.startsWith("/booking")) {
            return [
                { type: "send", label: "مواعيد متاحة", value: "أوقات متاحة مكياج 2025-12-20" },
                { type: "send", label: "قائمة الأسعار", value: "عرض قائمة الأسعار" },
    ...
    const smartReply = (
        ...
        if (
            ...
            (/عرض/i.test(qRaw) && !/عرض\s*قائمة\s*الاسعار|عرض\s*قائمة\s*الأسعار/i.test(qRaw))
        ) {
        ...
        if (/(ابي حجز|ابغى حجز|احجز|حجز|موعد)/i.test(qRaw)) {
            ...
            if (service && !date) {
                return {
                    ...
                    actions: [
                        { type: "send", label: "مثال جاهز", value: `أوقات متاحة ${service} 2025-12-20` },
                        { type: "route", label: "احجزي الآن", value: "/booking" },
                    ],
                };
            }
        ...
        if (/^(الشعر|شعر|قسم الشعر)$/i.test(qRaw.trim())) {
            return {
                ...
                actions: [
                    ...
                    { type: "send", label: "مواعيد متاحة", value: "أوقات متاحة الشعر 2025-12-20" },
                    { type: "send", label: "قائمة الأسعار", value: "عرض قائمة الأسعار" },
                ],
                showAllPricesBtn: true,
            };
    ...
    const handleSend = (custom?: string) => {
        ...
        setMessages((prev) => [...prev, { id: Date.now(), sender: "user", text: sendText }]);
        setInput("");
        ...
        <>
            ...
            <div className={`chatbot-container ${open ? "open" : ""}`}>
                <div className="chatbot-header">
                    ...
                    <span className="chatbot-close" onClick={() => setOpen(false)} title="إغلاق">
                        ×
                    </span>
                </div>
                ...
                {open && (
                    <div className="chatbot-body">
                                    ...
                                    {msg.actions?.length ? (
                                        <div className="suggestions-list">
                                            {msg.actions.map((a, idx) => (
                                                <button key={idx} className="suggestion-btn" onClick={() => handleAction(a)}>
                                                    {a.label}
                                                </button>
                                            ))}
                                        </div>
                        ...
                        <div className="chatbot-input-row">
                            <div className={`input-container${input.trim() ? " active" : ""}`}>
                                <input
                                    type="text"
                                    value={input}
