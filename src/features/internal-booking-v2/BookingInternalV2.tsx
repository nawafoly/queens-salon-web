...
export default function BookingInternalV2({ language = "ar" }: { language?: DashboardLanguage }) {
  ...
  const visibleServices = useMemo(() => {
    ...
    return services.filter((service) => {
      ...
      const displayTitle = bookingsText(language, serviceTitle(service));
            ...
            <main className="bk2-main-card">
              {step === 1 ? (
                ...
                <section className="bk2-services-step">
                  ...
                  <div className="bk2-section-tabs">{sections.map((section) => <button key={section.id} className={selectedSectionId === String(section.id) ? "is-active" : ""} onClick={() => setSelectedSectionId(String(section.id))}>{bookingsText(language, catalogLabel(section, "قسم"))}</button>)}</div>
                  {categories.length ? <div className="bk2-category-tabs"><button className={!selectedCategoryId ? "is-active" : ""} onClick={() => setSelectedCategoryId("")}>{t("الكل")}</button>{categories.map((category) => <button key={category.id} className={selectedCategoryId === String(category.id) ? "is-active" : ""} onClick={() => setSelectedCategoryId(String(category.id))}>{bookingsText(language, catalogLabel(category, "تصنيف"))}</button>)}</div> : null}
                    ...
                    {cart.map((service, index) => {
                      ...
                      return (
                        <article key={key} className={`bk2-schedule-item ${selection?.time ? "is-complete" : ""}`}>
                          <header><span>{index + 1}</span><div><strong>{bookingsText(language, serviceTitle(service))}</strong><small>{serviceDuration(service) || 30} {t("دقيقة")}</small></div>{selection?.time ? <em>✓ {t("مكتمل")}</em> : null}</header>
