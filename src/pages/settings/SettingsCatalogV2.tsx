            ...
            {composerMode === "section" ? (
              <div className="settings-catalog-v2-composer">
                ...
                <div className="settings-catalog-v2-form-grid">
                  ...
                  <Field label="الترتيب">
                    <input className="dsv2-input" type="number" value={newSection.order} onChange={(event) => setNewSection((previous) => ({ ...previous, order: Number(event.target.value || 0) }))} />
                  </Field>
                </div>
              ...
              <div className="settings-catalog-v2-composer">
                ...
                <div className="settings-catalog-v2-form-grid">
                  ...
                  <Field label="المدة (دقيقة)">
                    <input className="dsv2-input" type="number" min={5} value={newService.durationMin} onChange={(event) => setNewService((previous) => ({ ...previous, durationMin: parseNumberInput(event.target.value, previous.durationMin) }))} />
                  </Field>
                  <Field label="السعر">
                    <input className="dsv2-input" type="number" min={0} value={newService.price} onChange={(event) => setNewService((previous) => ({ ...previous, price: parseNumberInput(event.target.value, previous.price) }))} />
                  </Field>
                  <Field label="سعر الموسم">
                    <input className="dsv2-input" type="number" min={0} value={newService.seasonPrice ?? ""} onChange={(event) => setNewService((previous) => ({ ...previous, seasonPrice: event.target.value === "" ? null : parseNumberInput(event.target.value, Number(previous.seasonPrice || 0)) }))} />
                  </Field>
                </div>
      ...
      {activeCatalogPanel === "packages" ? (
        <section className="dsv2-card dsv2-card--padded settings-catalog-v2-panel">
          ...
          <div className="settings-catalog-v2-form-grid settings-catalog-v2-form-grid--packages">
            ...
            <Field label="عدد الجلسات"><input className="dsv2-input" type="number" min={1} value={packageSessionsCount} onChange={(event) => setPackageSessionsCount(Number(event.target.value))} /></Field>
            <Field label="السعر"><input className="dsv2-input" type="number" min={0} value={packagePrice} onChange={(event) => setPackagePrice(Number(event.target.value))} /></Field>
            <Field label="مدة الصلاحية بالأيام"><input className="dsv2-input" type="number" min={1} value={packageValidityDays} onChange={(event) => setPackageValidityDays(event.target.value)} placeholder="بدون انتهاء" /></Field>
            <Field label="ترتيب العرض"><input className="dsv2-input" type="number" min={0} value={packageSortOrder} onChange={(event) => setPackageSortOrder(Number(event.target.value))} /></Field>
          ...
          <div className="settings-catalog-v2-detail">
              ...
              <>
                {activeTab === "overview" ? (
                  <div className="settings-catalog-v2-form-grid">
                    ...
                    <Field label="الترتيب"><input className="dsv2-input" type="number" value={mode === "edit" ? sectionDraft?.order ?? 0 : selectedSectionLive.order} disabled={mode !== "edit"} onChange={(event) => setSectionDraft((previous) => previous ? { ...previous, order: Number(event.target.value || 0) } : previous)} /></Field>
            ...
            {activeListMode === "services" && selectedServiceLive ? (
              <>
                ...
                {activeTab === "pricing" ? (
                  <div className="settings-catalog-v2-form-grid">
                    <Field label="السعر"><input className="dsv2-input" type="number" min={0} value={mode === "edit" ? serviceDraft?.price ?? 0 : selectedServiceLive.price} disabled={mode !== "edit"} onChange={(event) => setServiceDraft((previous) => previous ? { ...previous, price: parseNumberInput(event.target.value, previous.price) } : previous)} /></Field>
