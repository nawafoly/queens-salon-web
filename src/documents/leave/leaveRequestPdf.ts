...
export async function buildLeaveRequestPdfBytes(request: EmployeeRequest) {
  ...
  const blackLogo = logoSource ? documentImageCanvas(logoSource, true) : null;
  ...
  drawLeavePdfText(context, data.title, canvas.width / 2, 94, 280, 34, 900, "#111", "center");
  context.beginPath();
  context.moveTo(LEAVE_PDF_CANVAS.margin + 34, 190);
  context.lineTo(canvas.width - LEAVE_PDF_CANVAS.margin - 34, 190);
  context.strokeStyle = "#222";
  context.stroke();
  drawLeavePdfText(context, `رقم الطلب: ${data.requestNumber}`, right, 212, 380, 17, 800, "#444");
  ...
  data.leaveTypeOptions.forEach((option, index) => {
    ...
    if (option.checked) {
      drawLeavePdfText(context, "✓", x + optionWidth - 12, y + 2, 20, 21, 900, "#111", "center");
    }
    drawLeavePdfText(context, option.label, x + optionWidth - 40, y + 3, optionWidth - 44, 17, 800);
  });

  y += 62;
  drawLeavePdfText(context, data.addressee, right, y, canvas.width - 220, 19, 900);
  y += 36;
  drawLeavePdfText(context, LEAVE_DOCUMENT_TEXT.honored, right, y, 280, 18, 700);
  y += 34;
  drawLeavePdfText(context, LEAVE_DOCUMENT_TEXT.greeting, right, y, 430, 18, 700);
  y += 42;
  ...
  drawLeavePdfText(context, "سبب الإجازة", right, y, 260, 15, 700, "#444");
  context.beginPath();
  context.moveTo(LEAVE_PDF_CANVAS.margin + 42, y + 76);
  context.lineTo(right, y + 76);
  context.stroke();
  drawLeavePdfWrappedText(context, data.reason, right, y + 24, canvas.width - 220, 17, 600, 2, 26);
  y += 104;

  if (data.notes !== "—") {
    drawLeavePdfText(context, "ملاحظات", right, y, 260, 15, 700, "#444");
    context.beginPath();
    context.moveTo(LEAVE_PDF_CANVAS.margin + 42, y + 70);
    context.lineTo(right, y + 70);
    context.stroke();
    drawLeavePdfWrappedText(context, data.notes, right, y + 24, canvas.width - 220, 17, 600, 2, 25);
    y += 94;
  }
  ...
  context.stroke();
  y += 26;
  drawLeavePdfText(context, LEAVE_DOCUMENT_TEXT.managerTitle, right, y, 360, 20, 900);
  y += 34;
  ...
  data.managerDecisionOptions.forEach((option, index) => {
    ...
    if (option.checked) {
      drawLeavePdfText(context, "✓", x + 284, y, 20, 21, 900, "#111", "center");
    }
    drawLeavePdfText(context, option.label, x + 260, y + 2, 230, 17, 800);
  });
  y += 50;
  drawLeavePdfText(context, LEAVE_DOCUMENT_TEXT.decisionNote, right, y, 280, 15, 700, "#444");
  drawLeavePdfWrappedText(context, data.managerDecisionNote, right, y + 24, canvas.width - 220, 17, 600, 2, 25);

  drawLeavePdfText(context, LEAVE_DOCUMENT_TEXT.copyNote, right, canvas.height - 86, 500, 12, 500, "#777");
